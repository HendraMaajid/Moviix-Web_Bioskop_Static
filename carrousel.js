...
function setVideoClasses() {
  var input = document
      .getElementById("video-caroussel")
...
function getSibling(elem, num, target) {
  ...
  for (var i = 0; i < num; i++) {
    if (tempElem !== null) {
      tempElem =
        target === "next"
          ? tempElem.nextElementSibling
          : tempElem.previousElementSibling
    }
  }
  return tempElem
}
...
function selectVideo(target) {
  ...
  if (getSibling(input, 2, target) === null) {
    elem =
      target === "next"
        ? getFirstChild(input)
        : getSibling(getLastChild(input), 1, "prev")
  } else {
...
function setActiveDot() {
  var navItems = document
      .getElementsByClassName("video-carousel-nav")[0]
      .querySelectorAll("li > label"),
    thumb = document
      .getElementById("video-caroussel")
      .querySelector("input:checked").nextElementSibling
...
function onOpenFrame(e) {
  var frame = document
    .getElementById(e.currentTarget.getAttribute("for"))
    .nextElementSibling.querySelector("iframe")
